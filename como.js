...
const I18N = {
  es: {
    ...
    m1b: 'Tu volado hace cola en una <strong>máquina real</strong>. Hay una fila de trabajos de físicos, universidades y empresas, y el tuyo se forma atrás. Correrlo son unos segundos; la espera en la fila puede ser de minutos o de horas. La pantalla no está fingiendo: está esperando de verdad.',
    m2a: '“Es millones de veces más rápida que una computadora normal.”',
    m2b: 'Para <strong>casi todo</strong>, es ridículamente más lenta. Es más rápida solo en un puñado de problemas muy específicos, y ni en esos ha ganado todavía de forma útil. Tu celular hace este mismo volado en 0.00001 ms. La máquina de IBM se tarda unos segundos en correr tus 1,000 tiros, más la fila.',
    ...
    adsP1: 'Tu volado gasta como <strong>3 segundos</strong> de máquina. Fuera de la cuota gratis, eso son <strong>$4.80 dólares</strong>. Un anuncio deja centésimas de centavo. Harían falta <strong>unas 3,200 visitas</strong> para pagar un solo volado.',
    ...
    shareP2: 'Cada volado manda su propio job y quema unos <strong>~3 segundos</strong> de máquina, así que el mes entero da para unos <strong>200</strong>. Hay un tope de <strong>8 al día</strong> pa que un bot no se queme el mes en una tarde. Si se acaban, la página te lo dice: se acabó, vuelve mañana (o el mes que entra).',
    ...
    fleetP1: 'Cada pregunta manda un <strong>job nuevo</strong>, en ese momento, a la máquina que esté <strong>operativa</strong> (no en mantenimiento) y con <strong>menos cola</strong> —eso es <span class="mono">service.least_busy()</span>—. Tu volado hace su propia fila y corre en vivo: por eso puede tardar de segundos a horas. Y por eso el nombre puede cambiar entre pregunta y pregunta: siempre se lleva la que esté más libre.',
    fleetP2: 'Por eso la tabla del resultado te dice en qué máquina corrió tu job y cuánto tardó: no es adorno, es la prueba de que tu bit salió de hardware real y no de un <span class="mono">Math.random()</span>. Si tu job cayó en una con 60 trabajos formados adelante, tardó: hay físicos de verdad usando esa misma máquina, y su experimento probablemente importa más que tu duda.',
    ...
    fleetP3: 'La flota cambia: IBM prende y apaga máquinas cada pocos meses. Por eso el sitio nunca trae un nombre escrito a mano, pregunta cuáles hay antes de mandar cada job.',
    ...
    tr4: 'Cuánto tarda', tr4b: 'unos segundos el job, más la cola',
  ...
  en: {
    ...
    m1b: 'Your flip waits in line on a <strong>real machine</strong>. There is a queue of jobs from physicists, universities and companies, and yours gets in at the back. Running it takes seconds; the wait in line can be minutes or hours. The screen is not pretending: it is genuinely waiting.',
    m2a: '“It is millions of times faster than a normal computer.”',
    m2b: 'For <strong>almost everything</strong>, it is absurdly slower. It is faster on a handful of very specific problems, and it has not usefully won even on those yet. Your phone does this same coin flip in 0.00001 ms. The IBM machine takes a few seconds to run your 1,000 shots, plus the queue.',
    ...
    f3k: 'What one ad brings in', f3n: 'per visit, roughly, with Human traffic.',
    adsP1: 'Your flip burns about <strong>3 seconds</strong> of machine time. Outside the free quota that is <strong>$4.80 dollars</strong>. An ad earns hundredths of a cent. It would take <strong>around 3,200 visits</strong> to pay for a single flip.',
    ...
    shareP2: 'Each flip sends its own job and burns about <strong>~3 seconds</strong> of machine time, so the whole month is good for around <strong>200</strong>. There is a cap of <strong>8 a day</strong> so a bot cannot burn the month in one afternoon. When they run out, the page tells you: it is gone, come back tomorrow (or next month).',
    ...
    fleetP1: 'Each question sends a <strong>new job</strong>, right then, to whichever machine is <strong>operational</strong> (not in maintenance) and has the <strong>shortest queue</strong> —that is <span class="mono">service.least_busy()</span>—. Your flip waits its own line and runs live: that is why it can take anywhere from seconds to hours. And that is why the name can change from question to question: it always goes to whichever is freest.',
    fleetP2: 'That is why the results table tells you which machine ran your job and how long it took: it is not decoration, it is the proof your bit came from real hardware and not a <span class="mono">Math.random()</span>. If your job landed on one with 60 jobs ahead of it, it was slow: there are real physicists on that same machine, and their experiment probably matters more than your question.',
    ...
    fleetP3: 'The fleet changes: IBM turns machines on and off every few months. That is why the site never hardcodes a name, it asks which ones exist before sending each job.',
    ...
    tr4: 'How long it takes', tr4b: 'a few seconds per job, plus a queue',
